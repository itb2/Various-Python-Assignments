...
def write_words(file, words):
    ...
    for line in words:           #This Function not only prints to the console, but opens up a file
        for w in line:           #and prints to that file
            file.write(w +" "),
        file.write("\n")
...
def transform(words, func):
    ...
    for l in range(len(newWords)):
        #this function has been corrected to apply the transformation to every word on every line of the text.
...
def transform_file ():
    ...
    if file == None:
        return
    words = get_words(file)            #this function applies the transformation that is chosen to each word in file
    file.close()
